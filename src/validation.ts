...
export const getStepErrors = (data: FormData): Record<number, string[]> => {
  ...
  if (data.massOutNeeded && (!data.massesOut.length || data.massesOut.some(x => !x.type || x.quantity <= 0 || !x.unit))) add(6, 'Fyll i alla massor som ska köras bort')
  if (data.materialInNeeded && (!data.materialsIn.length || data.materialsIn.some(x => !x.type || x.quantity <= 0 || !x.unit))) add(7, 'Fyll i allt material till arbetsplatsen')
