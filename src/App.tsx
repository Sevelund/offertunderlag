...
const stepNames = ['Grunduppgifter', 'Omfattning', 'Grävmaskin', 'Dumper och hjullastare', 'Padda och utrustning', 'Massor från platsen', 'Material till platsen', 'Övrigt material', 'Syfte', 'Genomförande', 'Förutsättningar', 'Övrig information', 'Bilder', 'Sammanställning']
function loadData(): FormData {
  const initial = createInitialData()
  try {
    const requestedArchiveId = archiveIdFromUrl()
    const archived = requestedArchiveId ? loadArchivedForm(requestedArchiveId) : null
    const saved = requestedArchiveId ? (archived ? JSON.stringify(archived) : null) : localStorage.getItem(ACTIVE_STORAGE_KEY)
...
export default function App() {
  ...
  const [sitePassword, setSitePassword] = useState(() => sessionPassword(SITE_AUTH_KEY))
  ...
  useEffect(() => {
    if (!siteUnlocked || !sitePassword) return
    let cancelled = false
    const sync = async () => {
      try {
        await migrateLocalArchives(sitePassword)
        if (archiveId) {
          const archived = await loadRemoteArchive(archiveId, sitePassword)
          if (!cancelled) setData(archived)
        }
        const forms = await listRemoteArchives(sitePassword)
        if (!cancelled) { setSavedForms(forms); setStorageError('') }
      } catch (error) {
        if (!cancelled) setStorageError(error instanceof Error ? error.message : 'Den gemensamma lagringen kunde inte nås.')
      } finally {
  ...
  }, [siteUnlocked, sitePassword, archiveId])
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const save = async () => {
        try {
          localStorage.setItem(archiveId ? archiveKey(archiveId) : ACTIVE_STORAGE_KEY, JSON.stringify(data))
          if (archiveId && sitePassword) await saveRemoteArchive(archiveId, data, sitePassword)
          setStorageError('')
        } catch { setStorageError('Formuläret kunde inte sparas. Kontrollera anslutningen eller ta bort någon stor bild.') }
      }
      save()
    }, archiveId ? 900 : 250)
    return () => window.clearTimeout(timer)
  }, [data, archiveId, archiveReady, sitePassword])
  ...
  const startNew = () => {
    ...
    setArchiveId(''); setArchiveReady(true); setLastSaved(null); setData(createInitialData()); setStep(1)
  }
  ...
  const makePdf = async () => {
    ...
    try {
      ...
      if (archiveId) {
        ...
        await saveRemoteArchive(archiveId, data, sitePassword)
        ...
        try { saved = saveArchivedForm(data); await saveRemoteArchive(saved.id, data, sitePassword) }
        catch (error) { if (activeBackup) localStorage.setItem(ACTIVE_STORAGE_KEY, activeBackup); throw error }
      }
      history.replaceState({}, '', `${location.origin}${location.pathname}`)
      setArchiveId(''); setArchiveReady(true); setLastSaved(saved); setSavedForms(await listRemoteArchives(sitePassword)); setCopied(false); setData(createInitialData()); setStep(1); setShowStepErrors(false)
  ...
  const removeSavedForm = async (id: string) => {
    if (!confirm('Vill du radera det sparade formuläret? Detta går inte att ångra.')) return
    try {
      await deleteRemoteArchive(id, sitePassword)
      deleteArchivedForm(id)
      setSavedForms(await listRemoteArchives(sitePassword))
      if (lastSaved?.id === id) setLastSaved(null)
    } catch (error) { setStorageError(error instanceof Error ? error.message : 'Formuläret kunde inte raderas.') }
  }
  ...
  return <>
    ...
    <main>
      ...
      {lastSaved && <div className="saved-copy"><div><b>PDF-filen är skapad och formuläret har rensats.</b><span>Formuläret är sparat och kan öppnas från alla enheter.</span></div><div className="saved-actions"><a href={lastSaved.url}>Öppna sparat formulär</a><button type="button" onClick={copySavedUrl}>{copied ? 'Webbadressen är kopierad' : 'Kopiera webbadress'}</button></div></div>}
      ...
      {step === 2 && <Section title="Arbetets omfattning" intro="Ange beräknad tid och hur personalbehovet fördelas.">
        <Field label="Hur många arbetsdagar beräknas arbetet ta?" required hint="Ange antal hela dagar."><NumberInput min={1} value={data.totalDays} onChange={value => set('totalDays', value)} /></Field>
        <h3>Personalperioder</h3>{data.personnel.map((x, i) => <RepeaterCard key={x.id} title={`Period ${i + 1}`} onRemove={() => removeItem('personnel', i)}><div className="grid two"><Field label="Antal personer"><NumberInput min={1} value={x.people} onChange={value => patchItem('personnel', i, { people: value })} /></Field><Field label="Antal dagar"><NumberInput min={1} value={x.days} onChange={value => patchItem('personnel', i, { days: value })} /></Field></div></RepeaterCard>)}
      ...
      {step === 4 && <Section title="Dumper och hjullastare"><Field label="Behövs dumper eller hjullastare?"><select value={data.loaderChoice} onChange={e => { const choice = e.target.value; let machines = data.machines; if (choice !== 'Nej' && !machines.length) machines = choice === 'Både dumper och hjullastare' ? [blankMachine('Dumper'), blankMachine('Hjullastare')] : [blankMachine(choice)]; setData(d => ({ ...d, loaderChoice: choice, machines })) }}><option>Nej</option><option>Dumper</option><option>Hjullastare</option><option>Både dumper och hjullastare</option></select></Field>
        {data.loaderChoice !== 'Nej' && <>{data.machines.map((x, i) => <RepeaterCard key={x.id} title={`Maskin ${i + 1}`} onRemove={() => removeItem('machines', i)}><div className="grid two"><Field label="Typ"><select value={x.type} onChange={e => patchItem('machines', i, { type: e.target.value })}><option value="">Välj</option><option>Dumper</option><option>Hjullastare</option></select></Field><Field label="Storlek eller modell" required><input value={x.size} onChange={e => patchItem('machines', i, { size: e.target.value })} /></Field><Field label="Antal dagar"><NumberInput min={1} value={x.days} onChange={value => patchItem('machines', i, { days: value })} /></Field><Field label="Egen eller hyrd">{ownership(x.ownership, v => patchItem('machines', i, { ownership: v }))}</Field><Field label="Transport till"><YesNo value={x.transportTo} onChange={v => patchItem('machines', i, { transportTo: v })} /></Field><Field label="Transport från"><YesNo value={x.transportFrom} onChange={v => patchItem('machines', i, { transportFrom: v })} /></Field></div><Field label="Kommentar"><textarea value={x.comment} onChange={e => patchItem('machines', i, { comment: e.target.value })} /></Field></RepeaterCard>)}<button type="button" className="add" onClick={() => addItem('machines', blankMachine())}>+ Lägg till maskin</button></>}
      </Section>}
      ...
      {step === 5 && <Section title="Padda och övrig utrustning">
        ...
        {data.needsEquipment && <>{data.equipment.map((x, i) => <RepeaterCard key={x.id} title={`Utrustning ${i + 1}`} onRemove={() => removeItem('equipment', i)}><div className="grid two"><Field label="Typ"><select value={x.type} onChange={e => patchItem('equipment', i, { type: e.target.value })}><option value="">Välj</option>{equipmentTypes.map(v => <option key={v}>{v}</option>)}</select></Field>{x.type === 'Annat' && <Field label="Beskriv utrustningen"><input value={x.customType} onChange={e => patchItem('equipment', i, { customType: e.target.value })} /></Field>}<Field label="Storlek eller modell"><input value={x.size} onChange={e => patchItem('equipment', i, { size: e.target.value })} /></Field><Field label="Antal"><NumberInput min={1} value={x.quantity} onChange={value => patchItem('equipment', i, { quantity: value })} /></Field><Field label="Antal dagar"><NumberInput min={1} value={x.days} onChange={value => patchItem('equipment', i, { days: value })} /></Field><Field label="Egen eller hyrd">{ownership(x.ownership, v => patchItem('equipment', i, { ownership: v }))}</Field><Field label="Behövs transport?"><YesNo value={x.transport} onChange={v => patchItem('equipment', i, { transport: v })} /></Field></div><Field label="Kommentar"><textarea value={x.comment} onChange={e => patchItem('equipment', i, { comment: e.target.value })} /></Field></RepeaterCard>)}<button type="button" className="add" onClick={() => addItem('equipment', blankEquipment())}>+ Lägg till utrustning</button></>}
      </Section>}

      {step === 7 && <Section title="Material och massor till arbetsplatsen"><WeightCalculator /><Field label="Ska material eller massor transporteras till arbetsplatsen?"><YesNo value={data.materialInNeeded} onChange={v => { const yes = Boolean(v); setData(d => ({ ...d, materialInNeeded: yes, materialsIn: yes && !d.materialsIn.length ? [blankMaterialIn()] : d.materialsIn })) }} /></Field>{data.materialInNeeded && <>{data.materialsIn.map((x, i) => <RepeaterCard key={x.id} title={`Material ${i + 1}`} onRemove={() => removeItem('materialsIn', i)}><div className="grid two"><Field label="Materialtyp"><select value={x.type} onChange={e => patchItem('materialsIn', i, { type: e.target.value })}><option value="">Välj</option>{materialTypes.map(v => <option key={v}>{v}</option>)}</select></Field>{x.type === 'Annat' && <Field label="Beskriv materialet"><input value={x.customType} onChange={e => patchItem('materialsIn', i, { customType: e.target.value })} /></Field>}<Field label="Mängd"><NumberInput min={0} step={0.1} inputMode="decimal" value={x.quantity} onChange={value => patchItem('materialsIn', i, { quantity: value })} /></Field><Field label="Enhet"><select value={x.unit} onChange={e => patchItem('materialsIn', i, { unit: e.target.value })}>{units.map(v => <option key={v}>{v}</option>)}</select></Field><Field label="Antal lass eller leveranser"><NumberInput min={0} value={x.deliveries} onChange={value => patchItem('materialsIn', i, { deliveries: value })} /></Field><Field label="Leveranssätt"><select value={x.deliveryMethod} onChange={e => patchItem('materialsIn', i, { deliveryMethod: e.target.value })}>{deliveryMethods.map(v => <option key={v}>{v}</option>)}</select></Field><Field label="Önskad placering"><input value={x.placement} onChange={e => patchItem('materialsIn', i, { placement: e.target.value })} /></Field></div><Field label="Kommentar"><textarea value={x.comment} onChange={e => patchItem('materialsIn', i, { comment: e.target.value })} /></Field></RepeaterCard>)}<button type="button" className="add" onClick={() => addItem('materialsIn', blankMaterialIn())}>+ Lägg till material</button></>}
      </Section>}

      {step === 6 && <Section title="Massor från arbetsplatsen"><WeightCalculator /><Field label="Ska massor transporteras bort från arbetsplatsen?"><YesNo value={data.massOutNeeded} onChange={v => { const yes = Boolean(v); setData(d => ({ ...d, massOutNeeded: yes, massesOut: yes && !d.massesOut.length ? [blankMassOut()] : d.massesOut })) }} /></Field>{data.massOutNeeded && <>{data.massesOut.map((x, i) => <RepeaterCard key={x.id} title={`Massor ${i + 1}`} onRemove={() => removeItem('massesOut', i)}><div className="grid two"><Field label="Typ av massor"><select value={x.type} onChange={e => patchItem('massesOut', i, { type: e.target.value })}><option value="">Välj</option>{massTypes.map(v => <option key={v}>{v}</option>)}</select></Field>{x.type === 'Annat' && <Field label="Beskriv massorna"><input value={x.customType} onChange={e => patchItem('massesOut', i, { customType: e.target.value })} /></Field>}<Field label="Uppskattad mängd"><NumberInput min={0} step={0.1} inputMode="decimal" value={x.quantity} onChange={value => patchItem('massesOut', i, { quantity: value })} /></Field><Field label="Enhet"><select value={x.unit} onChange={e => patchItem('massesOut', i, { unit: e.target.value })}>{units.map(v => <option key={v}>{v}</option>)}</select></Field><Field label="Uppskattat antal lass"><NumberInput min={0} value={x.loads} onChange={value => patchItem('massesOut', i, { loads: value })} /></Field><Field label="Känd eller misstänkt förorening"><select value={x.contamination} onChange={e => patchItem('massesOut', i, { contamination: e.target.value })}><option>Nej</option><option>Ja</option><option>Okänt</option></select></Field><Field label="Känd mottagningsplats"><input value={x.destination} onChange={e => patchItem('massesOut', i, { destination: e.target.value })} /></Field></div><Field label="Kommentar"><textarea value={x.comment} onChange={e => patchItem('massesOut', i, { comment: e.target.value })} /></Field></RepeaterCard>)}<button type="button" className="add" onClick={() => addItem('massesOut', blankMassOut())}>+ Lägg till massor</button></>}
      </Section>}

      {step === 8 && <Section title="Övrigt material"><Field label="Behövs något annat material för arbetet?"><YesNo value={data.otherMaterialNeeded} onChange={v => { const yes = Boolean(v); setData(d => ({ ...d, otherMaterialNeeded: yes, otherMaterials: yes && !d.otherMaterials.length ? [blankOtherMaterial()] : d.otherMaterials })) }} /></Field>{data.otherMaterialNeeded && <>{data.otherMaterials.map((x, i) => <RepeaterCard key={x.id} title={`Material ${i + 1}`} onRemove={() => removeItem('otherMaterials', i)}><div className="grid two"><Field label="Material"><select value={x.material} onChange={e => patchItem('otherMaterials', i, { material: e.target.value })}><option value="">Välj</option>{otherMaterialTypes.map(v => <option key={v}>{v}</option>)}</select></Field>{x.material === 'Annat' && <Field label="Beskriv materialet"><input value={x.customMaterial} onChange={e => patchItem('otherMaterials', i, { customMaterial: e.target.value })} /></Field>}<Field label="Mängd"><NumberInput min={0} step={0.1} inputMode="decimal" value={x.quantity} onChange={value => patchItem('otherMaterials', i, { quantity: value })} /></Field><Field label="Enhet"><select value={x.unit} onChange={e => patchItem('otherMaterials', i, { unit: e.target.value })}>{units.map(v => <option key={v}>{v}</option>)}</select></Field><Field label="Dimension eller specifikation"><input value={x.specification} onChange={e => patchItem('otherMaterials', i, { specification: e.target.value })} /></Field></div><Field label="Kommentar"><textarea value={x.comment} onChange={e => patchItem('otherMaterials', i, { comment: e.target.value })} /></Field></RepeaterCard>)}<button type="button" className="add" onClick={() => addItem('otherMaterials', blankOtherMaterial())}>+ Lägg till material</button></>}
      </Section>}
      ...
      <div className="step-jump"><label htmlFor="step-jump">Gå direkt till steg</label><select id="step-jump" value={step} onChange={e => go(Number(e.target.value))}>{stepNames.map((name, index) => <option value={index + 1} key={name}>Steg {index + 1} – {name}</option>)}</select></div>
    </main>
    <footer>Sevelund AB · Sparade formulär kan nås från alla enheter</footer>
  </>
}
...
function Compactor({ title, active, setActive, days, setDays, ownershipValue, setOwnership, comment, setComment }: { title: string; active: boolean; setActive: (v: boolean) => void; days: number; setDays: (v: number) => void; ownershipValue: string; setOwnership: (v: string) => void; comment: string; setComment: (v: string) => void }) {
  return <div className="equipment-block"><Field label={`Behövs ${title.toLowerCase()}?`}><YesNo value={active} onChange={v => setActive(Boolean(v))} /></Field>{active && <div className="grid two"><Field label="Antal dagar"><NumberInput min={1} value={days} onChange={setDays} /></Field><Field label="Egen eller hyrd"><select value={ownershipValue} onChange={e => setOwnership(e.target.value)}><option>Egen</option><option>Hyrd</option></select></Field><Field label="Kommentar"><textarea value={comment} onChange={e => setComment(e.target.value)} /></Field></div>}</div>
}
...
function WeightCalculator() {
  ...
  return <>
    ...
    {open && <div className="modal-backdrop" role="presentation" onMouseDown={e => { if (e.target === e.currentTarget) setOpen(false) }}><section className="weight-modal" role="dialog" aria-modal="true" aria-labelledby="weight-title"><button type="button" className="weight-close" aria-label="Stäng" onClick={() => setOpen(false)}>×</button><h2 id="weight-title">Vikt- och lassgenerator</h2><p className="weight-intro">Räkna om volym till ungefärlig vikt och antal lastbilslass. Generatorn kan användas för material både till och från arbetsplatsen.</p>
      <div className="grid two"><Field label="Material"><select value={material} onChange={e => selectMaterial(e.target.value)}>{materialDensities.map(x => <option key={x.name}>{x.name}</option>)}</select></Field><Field label="Volym (m³)"><NumberInput min={0} step={0.1} inputMode="decimal" value={volume} onChange={setVolume} /></Field><Field label="Densitet (ton/m³)" hint="Ungefärligt värde som kan ändras."><NumberInput min={0.1} step={0.1} inputMode="decimal" value={density} onChange={setDensity} /></Field></div>
      <h3>Lastbilens kapacitet</h3><div className="grid two"><Field label="Minst ton per lass"><NumberInput min={1} step={0.5} inputMode="decimal" value={minLoad} onChange={setMinLoad} /></Field><Field label="Högst ton per lass"><NumberInput min={1} step={0.5} inputMode="decimal" value={maxLoad} onChange={setMaxLoad} /></Field></div>
